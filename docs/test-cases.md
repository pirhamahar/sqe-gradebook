...
| TC-07 | Reject duplicate roll number | Roll numbers must be unique | A student with roll number 107 exists | Create two students with roll number 107. | Duplicate roll number is rejected. | High | Negative | FAIL — duplicate roll numbers are accepted by the current code. GitHub Issue #2.
